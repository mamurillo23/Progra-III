...
def jugarKenken(ventana): #Recibe la ventana del menu y la destruye
    ...
    Button(ventana,text="Guardar\nJuego",command=lambda:rehacerJugadas(),bg="lightpink",font=("Helvetica",14)).place(x=300,y=500)
    Button(ventana,text="Cargar\nJuego",command=lambda:rehacerJugadas(),bg="lightpink",font=("Helvetica",14)).place(x=400,y=500)
    Button(ventana,text="Borrar",command=lambda:boton("","si"),bg="black",fg="white",font=("Helvetica",14)).place(x=60,y=520)

    def guardarJuego():
        global jugadas,juego
        guardado=open("kenken_configuración.dat").readlines()
    ...
    def posiblesJugadas():
        ...
        for llave in juego:
            if posc in juego[llave]:
                ...
                cantidad = len(juego[llaves])-1
                operaciones(operacion,cantidad)

    def operaciones(operacion,cantidad):
        pass
